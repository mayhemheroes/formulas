#!/usr/bin/env bash
#
# formulas/mayhem/build.sh — build the ELF launcher shims for the Atheris fuzz harness and the
# test runner. formulas (vinci1it2000/formulas) is a PURE-PYTHON package: there is no native code
# to compile or sanitize. The Atheris harness (mayhem/fuzz_time.py) is a `.py`, but Mayhem requires
# the target `cmd:` to be an ELF, so we compile a tiny C shim per Python entry point that exec()s
# `python3 <script>` (see mayhem/launcher.c). The Python deps (atheris, formulas + its runtime deps,
# ddt for the test suite) are installed into the image's system Python by the Dockerfile — they need
...
# The base exports $SANITIZER_FLAGS (ASan+UBSan, halting) for projects with compiled code; formulas
...
# Fuzz target: the preserved Atheris harness.
build_launcher fuzz-time      /mayhem/mayhem/fuzz_time.py
# Test oracle runner: runs the real unittest suite (driven by mayhem/test.sh through this ELF so the
# sabotage check can neuter it).
build_launcher formulas-tests /mayhem/mayhem/run_tests.py

echo "build.sh complete:"
ls -la /mayhem/fuzz-time /mayhem/formulas-tests
